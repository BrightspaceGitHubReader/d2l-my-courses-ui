...
$_documentContainer.innerHTML = `<dom-module id="d2l-filter-menu-tab-legacy">
		...
		<div hidden$="[[!_showContent]]">
			...
			<d2l-menu label="[[menuLabelText]]">
				<template is="dom-repeat" items="[[_allFilters]]">
					<d2l-filter-list-item-legacy enrollment-entity="[[item]]" selected="[[_checkSelected(item)]]">
					</d2l-filter-list-item-legacy>
				</template>
			</d2l-menu>
...
Polymer({
	...
	clear: function() {
		var items = this.$$('d2l-menu').querySelectorAll('d2l-filter-list-item-legacy');
