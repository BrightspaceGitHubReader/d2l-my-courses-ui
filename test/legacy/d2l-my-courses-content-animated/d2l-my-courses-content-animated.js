describe('d2l-my-courses-content-animated', function() {
	...
	describe('User interaction', function() {
		...
		it('should rescale the all courses view when it is opened', function(done) {
			...
			flush(() => {
				var allCoursesRescaleSpy = sandbox.spy(widget.$$('d2l-all-courses-legacy').$$('d2l-all-courses-segregated-content'), '_rescaleCourseTileRegions');
				clock.tick(100);
				expect(allCoursesRescaleSpy.called);
				widget.$$('d2l-all-courses-legacy').$$('d2l-all-courses-segregated-content')._rescaleCourseTileRegions.restore();
				done();
			});
		});
